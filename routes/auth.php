...
Route::group(['middleware' => config('fortify.middleware', ['web'])], function () use ($prefix) {
    $enableViews = config('fortify.views', true);

    // Authentication...
    if ($enableViews) {
        Route::get(RoutePath::for('login', '/login'), [AuthenticatedSessionController::class, 'create'])
            ->middleware(['webguest:'.$prefix.','.config('fortify.guard')])
            ->name('login');
    }
    ...
    Route::post(RoutePath::for('login', '/login'), [AuthenticatedSessionController::class, 'store'])
        ->middleware(array_filter([
            'webguest:'.$prefix.','.config('fortify.guard'),
            $limiter ? 'throttle:'.$limiter : null,
        ]))->name('login.store');

    Route::post(RoutePath::for('logout', '/logout'), [AuthenticatedSessionController::class, 'destroy'])
        ->middleware(['webauth'.":{$prefix}".config('fortify.guard')])
        ->name('logout');
    ...
    if (Features::enabled(Features::emailVerification())) {
        ...
        Route::get(RoutePath::for('verification.verify', '/email/verify/{id}/{hash}'), [VerifyEmailController::class, '__invoke'])
            ->middleware(['webauth'.":{$prefix}".config('fortify.guard'), 'signed', 'throttle:'.$verificationLimiter])
            ->name('verification.verify');

        Route::post(RoutePath::for('verification.send', '/email/verification-notification'), [EmailVerificationNotificationController::class, 'store'])
            ->middleware(['webauth'.":{$prefix}".config('fortify.guard'), 'throttle:'.$verificationLimiter])
    ...
    if (Features::enabled(Features::twoFactorAuthentication())) {
        ...
        $twoFactorMiddleware = Features::optionEnabled(Features::twoFactorAuthentication(), 'confirmPassword')
            ? ['webauth'.":{$prefix}".config('fortify.guard'), 'password.confirm']
            : ['webauth'.":{$prefix}".config('fortify.guard')];
    ...
    if (Features::enabled(Features::passkeys())) {
        ...
        $passkeyAuthMiddleware = ['webauth'.":{$prefix}".config('fortify.guard')];
        ...
        $passkeyGuestMiddleware = ['webguest:'.$prefix.','.config('fortify.guard'), ...$throttle];
