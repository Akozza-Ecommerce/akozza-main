...
import { edit, index, update } from '@/routes/prescriptions';
import type { Prescription, Customer } from '@/types';
import { Link, useForm } from '@inertiajs/react';

export default function Edit({ prescription, customers }: { prescription: Prescription, customers: Customer[] }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        right_distance_va: prescription.right_distance_va || '',
        right_axis: prescription.right_axis || '',
        right_add: prescription.right_add || '',
        right_near_va: prescription.right_near_va || '',
        left_sph: prescription.left_sph || '',
        left_cyl: prescription.left_cyl || '',
        left_distance_va: prescription.left_distance_va || '',
        left_axis: prescription.left_axis || '',
        left_add: prescription.left_add || '',
        left_near_va: prescription.left_near_va || '',
        ipd_distance: prescription.ipd_distance || '',
        ipd_near: prescription.ipd_near || '',
        receipt_source: prescription.receipt_source || '',
        ipd_source: prescription.ipd_source || '',
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(update(prescription).url);
    };
    ...
    return (
            ...
            <div className="flex flex-col gap-6 p-8">
                <PageHeader title="Edit Prescription" description="Update patient prescription details." />
                ...
                <form onSubmit={submit} className="space-y-8">
                    ...
                    <div className="flex gap-4">
                        <Button type="submit" disabled={processing}>
                            {processing ? 'Saving...' : 'Update Prescription'}
                        </Button>
